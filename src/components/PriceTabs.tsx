    ...
    <div className="flex justify-center">
      <Tabs defaultValue={packages[0]?.name} className="w-full p-2">
        <TabsList className={tabsListClass}>
          {packages.map((pkg) => (
            <TabsTrigger className="data-[state=active]:bg-sky-200 rounded-xl" key={pkg.name} value={pkg.name}>
              {pkg.name}
            </TabsTrigger>
          ))}
        </TabsList>
        {packages.map((pkg) => (
          <TabsContent key={pkg.name} value={pkg.name}>
            <CardContent className="flex flex-col shadow-md rounded-xl items-center p-2 md:p-4 dark:bg-zinc-950">
